...
// Simple rate limiting using file-based cache (in production, use Redis or database)
...
if (file_exists($attempts_file)) {
    ...
    if ($attempts_data && $attempts_data['count'] >= $max_attempts && 
        ...
        $retVal = array("error" => "Too many failed login attempts. Please try again later.");
        error_log("Login rate limit exceeded for IP: " . $client_ip);
...
if (!preg_match('/^[a-zA-Z0-9._-]{1,50}$/', $username)) {
    ...
    error_log("Login attempt with invalid username format: " . var_export($username, true) . " from IP: " . $_SERVER['REMOTE_ADDR']);
} else if (strlen($password) > 255) {
    // Prevent extremely long passwords that could cause DoS
    $retVal = array("error" => "Invalid password format.");
    error_log("Login attempt with excessive password length from IP: " . $_SERVER['REMOTE_ADDR']);
} else {
    // Use parameterized query approach - escape quotes and backslashes
    $safe_username = str_replace(array("'", "\\"), array("''", "\\\\"), $username);
    $safe_password = str_replace(array("'", "\\"), array("''", "\\\\"), $password);
    
    $query = "SELECT * FROM usergroup WHERE ugr_name = '" . $safe_username . "' AND ugr_password = '" . $safe_password . "'";
    ...
    if ($dbMgr->getRowCount() == 0) {
        ...
        error_log("Failed login attempt for username: $username from IP: " . $client_ip);
        ...
        if ($user['ugr_type_id'] != 511) { 
            ...
            // SECURITY: Track failed login attempts for wrong user type
            $attempts_data = array('count' => 1, 'first_attempt' => $current_time);
            if (file_exists($attempts_file)) {
                $existing_data = json_decode(file_get_contents($attempts_file), true);
                if ($existing_data) {
            ...
            file_put_contents($attempts_file, json_encode($attempts_data));
            error_log("Failed login attempt (wrong user type) for username: $username from IP: " . $client_ip);
        } else {

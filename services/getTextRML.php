...
// setup varibles and lookups
  ...
  $colInfoLookup = array();
  $retVal = array();
//  $userID = @$_REQUEST['userID'] ? $_REQUEST['userID']:13;
  ...
  $entityTypeList = (array_key_exists('entityList',$_REQUEST)? $_REQUEST['entityList']:
              "'text','textmetadata','grapheme','token','compound','lemma','attributiongroup','image',
              'baseline','segment','syllablecluster','sequence','edition','bibliography','annotation',
              'fragment','surface','part','item','attribution','date','era'");
  $entityLookup = array();
  $dbMgr->query("select table_name as entity,ordinal_position as order,column_name as column,data_type as type,
                        character_maximum_length as maxlength, udt_name as subtype,
                        CASE WHEN is_nullable = 'NO' THEN true ELSE false END as required
                from INFORMATION_SCHEMA.COLUMNS
                where table_name in ($entityTypeList) order by entity,ordinal_position;");
  while($row = $dbMgr->fetchResultRow()){
    $colInfoLookup[$row['column']] = $row;
  }
  ...
  if (is_array($textCKNs)) {// multiple text
    ...
    $condition = "";
    if (strpos($textCKNs,",")) {//string of multiple ckn's
...
//get text entity
  ...
  if (!$text || !$text->getID()) {
    echo "Texts for condition '$condition' not found aborting RML generation.";
    return;
  }
...
//get all editions
  ...
  if (!$editions || $editions->getCount() == 0) {
    echo "<error>No editions available for Text ".$text->getCKN().", aborting RML generation.</error>";
    return;
  }

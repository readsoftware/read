...
*/
  ...
  */
  ...
  if (@$argv) {
    ...
    if (@$cmdParams['-ckn']) $_REQUEST['ckn'] = $cmdParams['-ckn'];
...
// check for cache
  if (count($textIDs) == 1 && USECACHE) {// text ids
    ...
    foreach ($lines as $line) {
      ...
      if ($linID && !array_key_exists($linID, $entities['lin'])) {
        ...
        $entities['lin'][$linID] = array( 'spnIDs'=> $linSpnIDs,
                               ...
                               'mask' => $line->getMask());
        if (count($linSpnIDs) && @$linSpnIDs[count($linSpnIDs)-1] && @$entities['spn'][$linSpnIDs[count($linSpnIDs)-1]] ) {

...
//*******************************************************************
/**
  * Runs class which is an iterating container of runs
  *
  * <code>
  ...
  * $run = $runs->current();
  * $key = $runs->key();
  * echo " run $key has URL ".$run->getURL();
  * </code>
  *
  * @author      Stephen White  <stephenawhite57@gmail.com>
  */

  class Runs extends EntityIterator {
    ...
    /**
    * Create a Runs iterator, optionally setting the offset and pagesize
    ...
    */
    public function __construct( $condition = "", $sort = "run_id", $offset = 0, $pageSize = 20) {
      parent::__construct("run","run_id");
      $this->_autoAdvancePage = true;
      $this->_pageSize = $pageSize;
      $this->_offset = $offset;
      if ($condition) $this->_condition = $condition;
//      $this->_security = isSysAdmin()?null:" (".getUserID()."= run_owner_id or ".getUserID()." = ANY (\"run_visibility_ids\"))";
      $this->_security = parent::getEntityAccessCondition("run");
      $this->_sort = $sort;
    ...
    /**
    * getRuns - array of runs from the current query
    *
    * @return array returns an run array for the current page size (default is 20)
    */

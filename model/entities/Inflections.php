...
//*******************************************************************
/**
  * Inflections class which is an iterating container of inflections
  *
  * <code>
  * require_once 'Inflections.php';
  *
  * $inflections = new Inflections(10,5);
  * $inflection = $inflections->current();
  * $key = $inflections->key();
  * echo " inflection $key is ".$inflection->getInflection();
  * </code>
  *
  * @author      Stephen White  <stephenawhite57@gmail.com>
  */

  class Inflections extends EntityIterator {
    ...
    /**
    * Create an Inflections iterator, optionally setting the offset and pagesize
    ...
    */
    public function __construct( $condition = "", $sort = "inf_id", $offset = 0, $pageSize = 20) {
      parent::__construct("inflection","inf_id");
...
//      $this->_security = isSysAdmin()?null:" (".getUserID()."= lem_owner_id or ".getUserID()." = ANY (\"lem_visibility_ids\"))";
      $this->_security = parent::getEntityAccessCondition("inf");
      $this->_sort = $sort;
    ...
    /**
    * Inflections - array of inflections from the current query
    *
    * @return array returns a Inflection array for the current page size (default is 20)
    */

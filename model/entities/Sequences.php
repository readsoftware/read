...
//*******************************************************************
/**
  * Sequences class which is an iterating container of sequences
  *
  * <code>
  * require_once 'Sequences.php';
  *
  * $sequences = new Sequences(10,5);
  * $sequence = $sequences->current();
  * $key = $sequences->key();
  * echo " sequence $key has objects - ".$sequence->getEntityIDs(true);
  * </code>
  *
  * @author      Stephen White  <stephenawhite57@gmail.com>
  */

  class Sequences extends EntityIterator {
    ...
    /**
    * Create an Sequences iterator, optionally setting the offset and pagesize
    ...
    */
    public function __construct( $condition = "", $sort = "seq_id", $offset = 0, $pageSize = 20) {
      parent::__construct("sequence","seq_id");
      $this->_pageSize = $pageSize;
      $this->_offset = $offset;
      if ($condition) $this->_condition = $condition;
//      $this->_security = isSysAdmin()?null:" (".getUserID()."= seq_owner_id or ".getUserID()." = ANY (\"seq_visibility_ids\"))";
      $this->_security = parent::getEntityAccessCondition("seq");
      $this->_sort = $sort;
    ...
    /**
    * Sequences - array of sequences from the current query
    *
    * @return array returns a Sequence array for the current page size (default is 20)
    */

...
//*******************************************************************
/**
  * Texts class which is an iterating container of texts
  *
  * <code>
  * require_once 'Texts.php';
  *
  * $texts = new Texts("",,10,5);
  * $text = $texts->current();
  * $key = $texts->key();
  * echo " text $key is ".$text->getTitle();
  * </code>
  *
  * @author      Stephen White  <stephenawhite57@gmail.com>
  */

  class Texts extends EntityIterator {

    //*******************************PRIVATE MEMBERS************************************
    //****************************CONSTRUCTOR FUNCTION***************************************

    /**
    * Create an Texts iterator, optionally setting the offset and pagesize
    ...
    */
    public function __construct( $condition = "", $sort = "txt_id", $offset = 0, $pageSize = 20) {
      parent::__construct("text","txt_id");
      $this->_autoAdvancePage = true;
      $this->_pageSize = $pageSize;
      $this->_offset = $offset;
      if ($condition) $this->_condition = $condition;
//      $this->_security = isSysAdmin()?null:" (".getUserID()."= txt_owner_id or ".getUserID()." = ANY (\"txt_visibility_ids\"))";
      $this->_security = parent::getEntityAccessCondition("txt");
      $this->_sort = $sort;
    ...
    *
    * @param int $index in texts
    * @return Text or NULL
    */
    public function getTextAt($index) {
      return (array_key_exists($index,$this->_entities)? $this->_entities[$index]:NULL);
    }

    /**
    * Texts - array of texts from the current query
    *

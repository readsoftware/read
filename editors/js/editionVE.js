...
/**
* insert line before or after the current line dependent on position
*
* @param function cbError Error callback function\n*/

  insertLine: function (cbError) {
    ...
    switch (startNode.nodeName) {
      ...
      case 'SPAN':
        ...
        break;
      default:
        DEBUG.log("gen","insertLine not able to detect position for "+this.edition.value);
        return;
    }
    if (!headerNode) {
        DEBUG.log("gen","insertLine not able to detect position for "+this.edition.value);
      ...
      if (posInsert == "before") {
        ...
        if (headerNode.prevUntil('.grpGra','.linebreak').attr('class').match(/split/)) {
          alert("Inserting line between lines sharing a token is not allowed. Please split the last token of previous line first.");
          return;
        }
        //first grpGra past header has context IDs
        refNode = headerNode.nextUntil('.grpGra').length?//if TCM nodes the count is non zero
                    headerNode.nextUntil('.grpGra').last().next() : headerNode.next();
      } else {
        //if not startHeader then check for split token on previous linebreak
        if (headerNode.nextUntil('br','.linebreak').attr('class').match(/split/)) {
          alert("Inserting line between lines sharing a token is not allowed. Please split the last token of this line first.");
          return;
        }
        //last grpGra before linebreak has context IDs
        refNode = headerNode.nextUntil('.linebreak','.grpGra').last();
      }
      context = refNode.attr('class').replace(/grpGra/,"")
                                      .replace(/ord\d+/,"")
                                      .replace(/ordL\d+/,"")
                                      .replace(/seg\d+/,"")
                                      .replace(/firstLine/,"")
                                      ...
                                      .replace(/\s+/g,",")
                                      .split(",");
      refDivSeqID = context.shift().substring(3);//!!!!Caution!!!!!  class order dependency
      refEntGID = context.shift().replace(/(.{3})(.*)/,"$1:$2");
      refLineSeqGID = (headerNode.get(0).className.match(/line(seq\d+)/)[1]).replace(/(.{3})(.*)/,"$1:$2");
    }
    //setup data
    insertdata={
      ednID: this.edition.id,
      refDivSeqID:refDivSeqID,
      refEntGID:refEntGID,
      //label:"newline 1",
      insPos: posInsert,
    ...
    DEBUG.log("gen","call to insertLine "+ insertdata.insPos +
                " line sequence id "+insertdata.refLineSeqID +
                " and new token "+ insertdata.insPos +
                " reference entity "+insertdata.refEntGID +
                " in text division sequence id " + insertdata.refDivSeqID);
    ...
    $.ajax({
        type:"POST",
        dataType: 'json',
        url: basepath+'/services/insertNewLine.php?db='+dbName,
        ...
        success: function (data, status, xhr) {
            if (typeof data == 'object' && data.success && data.entities) {
              ...
              newTokGID = data['tokGID'];
              newTextDivSeqGID = data['textDivSeqGID'];
              ednVE.replaceTag('seq'+refDivSeqID,newTextDivSeqGID.replace(':',''));
              newPhysLineSeqID = (data['physLineSeqGID']).substring(4);
              if (data['newPhysSeqID']) {
                ednVE.physSeq = ednVE.dataMgr.getEntity('seq',data['newPhysSeqID']);
              }
              ...
              DEBUG.log("data","after insertLine sequence dump\n" + DEBUG.dumpSeqData(newPhysLineSeqID,0,1,ednVE.lookup.gra));
              DEBUG.log("data","after insertLine sequence dump\n" + DEBUG.dumpSeqData(ednVE.textSeq.id,0,1,ednVE.lookup.gra));
              DEBUG.log("data","after insertLine sequence dump\n" + DEBUG.dumpSeqData(newTextDivSeqGID.substring(4),0,1,ednVE.lookup.gra));
              //calc render mapping for new token with walkTokenContext
              ednVE.walkTokenContext(newTokGID, newTextDivSeqGID.replace(':',''), true, null);
              // calcLineGraphemeLookups for new line ? needed?
              ednVE.calcLineGraphemeLookups(newPhysLineSeqID);
              //calc html for newline
              newLineHTML = ednVE.renderPhysicalLine(newPhysLineSeqID,null,null,1,newLineOrd);
              //insert line  and renumber
              ednVE.renderNewLine(newLineHTML[0],newLineOrd,data['lastReplaced']);
              //temporary redraw all
              //ednVE.renderEdition();
              //position sylED on new syllable
              if (typeof ednVE.sclEd != "undefined") {
                ednVE.sclEd.moveToSyllable(newSclID,'selectAll');
              }
              if (data.editionHealth) {
                DEBUG.log("health","***Insert Line***");

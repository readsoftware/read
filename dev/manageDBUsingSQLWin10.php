...
if (!$cmd && !$dbname && !$sqlfilename) {
    ...
    switch ($cmd) {
    case "restore":
            //WARNING this set of commands may shut of a connection 
            //causing a warning first connection to the new/restored db
            //This can be avoided by checking PHP.ini for 
            //pgsql.auto_reset_persistent and set it to On.
    ...
    case "create":
        ...
        if (runShellCommand($command, "CREATED $dbname database", "Aborting - failed to create database $dbname")) {
            ...
            if (runShellCommand($command, "Loaded $dbname database from $sqlFilePath$sqlfilename", "Aborting - failed to load database $dbname from $sqlFilePath$sqlfilename")) {
                ...
                runShellCommand(
                    $command, "GRANTED connection on $dbname database", 
                    "Aborting - failed to GRANTED connections to database $dbname"
                );
            }
        }
        break;
    ...
    case "snapshot":
        $command = $psqlPath."pg_dump -U '.$psqlUser.' --no-privileges --no-owner $dbname >".
                    " $sqlFilePath"."snapshot$sqlfilename";
        if (runShellCommand(
            $command, 
            "Dump $dbname database to $sqlFilePath"."snapshot$sqlfilename", 
            "Aborting - failed to dump database $dbname to $sqlFilePath"."snapshot$sqlfilename"
        )) {
